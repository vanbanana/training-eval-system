...
func (f *FakeLLM) Complete(ctx interface{ Deadline() (t1 interface{}, ok bool); Done() <-chan struct{}; Err() error; Value(key any) any }, messages []llm.ChatMessage, tools []llm.Tool) (*llm.ChatResponse, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
