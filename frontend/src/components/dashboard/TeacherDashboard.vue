...
const weekActivityTotal = computed(() => stats.value?.activity_7d?.reduce((s, d) => s + d.count, 0) ?? 0)
  ...
  <div v-else-if="stats" class="td">
    ...
    <div class="td-body">
      <div class="td-left">
        ...
        <div class="td-stats">
          <div class="td-st">
            <span class="td-ic td-ic-info"><ClipboardList /></span>
            <div>
              <label>待批改提交</label>
              <div class="td-sv">
                <b>{{ stats.pending_grading }}</b
                ><em :class="stats.pending_grading > 0 ? 'o' : 'g'">{{
                  stats.pending_grading > 0 ? '待处理' : '已清空'
                }}</em>
              </div>
            </div>
          </div>
          <div class="td-st">
            <span class="td-ic td-ic-primary"><CheckCheck /></span>
            <div>
              <label>本周已批改</label>
              <div class="td-sv">
                <b>{{ stats.graded_this_week }}</b
                ><em class="b">本周</em>
              </div>
            </div>
          </div>
          <div class="td-st">
            <span class="td-ic td-ic-success"><Gauge /></span>
            <div>
              <label>班级平均分</label>
              <div class="td-sv">
                <b>{{ stats.class_avg_score ?? '—' }}</b
                ><em v-if="avgScoreTier" :class="avgScoreTier.cls">{{ avgScoreTier.label }}</em>
              </div>
            </div>
          </div>
          <div class="td-st">
            <span class="td-ic td-ic-danger"><ShieldAlert /></span>
            <div>
              <label>疑似抄袭警告</label>
              <div class="td-sv">
                <b>{{ suspectCount }}</b
        ...
        <div class="td-mid">
          <div class="td-chart">
            ...
            <div class="td-chart-area">
              <!-- Peak badge -->
              <span
                v-if="peakIdx >= 0 && chartPoints.length"
                class="td-peak-badge"
                :style="{ left: `${(chartPoints[peakIdx].x / chartW) * 100}%` }"
              >
              ...
              <svg v-else :viewBox="`0 0 ${chartW} ${chartH}`" preserveAspectRatio="none" class="td-svg">
                ...
                <template v-for="(pt, i) in chartPoints" :key="i">
                  <circle
                    ...
                    :cy="pt.y"
                    :r="i === peakIdx ? 5 : 3"
                    :class="i === peakIdx ? 'chart-dot-peak' : 'chart-dot'"
                    :stroke-width="i === peakIdx ? 2 : 0"
                  />
                </template>
              </svg>
            </div>
        ...
        <div class="td-tasks">
          <div
            v-for="t in stats.recent_tasks.slice(0, 3)"
            :key="t.id"
            class="td-task"
            @click="go(`/teacher/tasks/${t.id}`)"
          >
            <div class="td-task-top">
              <span class="td-ic td-ic-sm td-ic-info"><FileText /></span
              ><em :class="t.status === 'published' ? 'b' : t.status === 'closed' ? 'g' : 'o'">{{
                t.status === 'published' ? '进行中' : t.status === 'closed' ? '已完成' : '待批改'
              }}</em>
            </div>
            <h4>{{ t.name }}</h4>
            ...
            <div class="td-bar">
              <div
                :class="{ done: t.status === 'closed' }"
                :style="{ width: `${t.total_students ? ((t.graded ?? 0) / t.total_students) * 100 : 0}%` }"
              ></div>
            </div>
      ...
      <div class="td-right">
        <div class="td-qa-card">
          ...
          <div class="td-qa-grid">
            <div class="td-qa" @click="go('/teacher/tasks/new')">
              <span class="td-ic td-ic-primary"><FilePlus /></span><span class="td-qa-t">创建实训任务</span
              ><small>快速布置新任务</small>
            </div>
            <div class="td-qa" @click="go('/teacher/tasks')">
              <span class="td-ic td-ic-info"><ClipboardCheck /></span><span class="td-qa-t">批改工作台</span
              ><small>高效批改学生实训任务</small>
            </div>
            <div class="td-qa" @click="go('/teacher/classes')">
              <span class="td-ic td-ic-success"><Users /></span><span class="td-qa-t">班级管理</span
              ><small>查看班级学生情况</small>
            </div>
            <div class="td-qa" @click="go('/profiles')">
              <span class="td-ic td-ic-accent"><PieChart /></span><span class="td-qa-t">评价看板</span
              ><small>多维度数据分析</small>
            </div>
          </div>
        </div>
        <div class="td-notif-card">
          <div class="td-sec-hd">
            <h3>重要通知</h3>
            <a @click="go('/notifications')">查看全部 &gt;</a>
          </div>
          <div class="td-notifs">
            <div v-for="n in stats.recent_notifications.slice(0, 3)" :key="n.id" class="td-nf">
              <div class="td-nf-d">
                <b>{{ n.created_at ? new Date(n.created_at).getDate() || '—' : '—' }}</b
                ><small>{{ n.created_at ? new Date(n.created_at).getMonth() + 1 + '月' : '' }}</small>
              </div>
...
.td-peak-arrow {
  ...
  border-top: 5px solid hsl(var(--info));
}
.td-chart-area {
  position: relative;
  height: 180px;
