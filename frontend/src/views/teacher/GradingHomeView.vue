...
<template>
  <AppShell>
    <BreadcrumbNav :items="[{ label: '工作台', to: '/dashboard' }, { label: '批改首页' }]" />
    ...
    <div v-if="data" class="grid grid-cols-4 gap-4 mb-6">
      <Card class="p-4 flex items-center gap-3.5">
        <span class="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-warning-soft text-warning">
          <FileText class="w-5 h-5" />
        </span>
        <div>
          <div class="text-2xl font-bold num-tabular text-ink">{{ data.summary.pending_ai_count }}</div>
          <div class="text-xs text-muted-foreground">待 AI 批改</div>
        </div>
      </Card>
      <Card class="p-4 flex items-center gap-3.5">
        ...
        <div>
          <div class="text-2xl font-bold num-tabular text-ink">{{ data.summary.scored_unconfirmed_count }}</div>
          <div class="text-xs text-muted-foreground">AI 已评分待确认</div>
        </div>
      </Card>
      <Card class="p-4 flex items-center gap-3.5">
        <span class="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-success-soft text-success">
          <CheckCircle2 class="w-5 h-5" />
        </span>
        <div>
          <div class="text-2xl font-bold num-tabular text-ink">{{ data.summary.confirmed_today_count }}</div>
          <div class="text-xs text-muted-foreground">今日已确认</div>
        </div>
      </Card>
      <Card class="p-4 flex items-center gap-3.5">
        ...
        <div>
          <div class="text-2xl font-bold num-tabular text-ink">{{ data.courses.length }}</div>
          <div class="text-xs text-muted-foreground">我的课程</div>
        </div>
      </Card>
    </div>
    ...
    <div v-else class="flex gap-4">
      ...
      <Card class="w-[200px] shrink-0 overflow-hidden">
        ...
        <div v-else class="flex flex-col">
          <button
            v-for="cl in currentCourse()?.classes"
            :key="cl.id"
            class="px-3 py-2.5 text-left text-sm hover:bg-accent-soft transition-colors"
            :class="cl.id === selectedClass ? 'bg-accent-soft text-accent-strong font-semibold' : ''"
            @click="selectedClass = cl.id"
          >
      ...
      <Card class="flex-1 overflow-hidden">
        ...
        <div v-else class="flex flex-col">
          <div
            v-for="t in currentClass()?.tasks"
            :key="t.id"
            class="p-4 border-b border-border last:border-b-0 flex justify-between items-center"
          >
            <div>
