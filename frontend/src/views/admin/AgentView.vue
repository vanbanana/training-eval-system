...
<template>
  <AppShell>
    ...
    <div class="tes-page-header mb-5">
      <div>
        <div class="flex items-center gap-3">
          ...
          <span v-if="llmStatus === 'ok'" class="inline-flex items-center gap-1 text-xs text-success"><!-- label -->
            <CheckCircle2 class="w-3 h-3" /> LLM 已配置
          </span>
          <span v-else-if="llmStatus === 'error'" class="inline-flex items-center gap-1 text-xs text-danger"><!-- label -->
    ...
    <!-- LLM Warning -->
    <div v-if="llmStatus === 'error'" class="mb-4 flex items-center gap-2 px-4 py-2.5 bg-warning-soft border border-warning/30 rounded-lg text-sm text-warning">
            ...
            <div v-if="messages.length === 0" class="flex-1 flex flex-col items-center justify-center text-center gap-4">
              ...
              <div class="grid grid-cols-2 gap-2 mt-2 w-full max-w-md">
                <button
                  ...
                  :key="q"
                  class="flex items-center gap-2 px-3.5 py-2.5 bg-surface-2 border border-border rounded-lg text-sm font-medium text-ink hover:border-primary hover:bg-primary-soft transition-colors"
                  @click="askQuick(q)"
                >
...
<!-- 样式已迁移到 styles/globals.css .tes-prose 统一类中 -->